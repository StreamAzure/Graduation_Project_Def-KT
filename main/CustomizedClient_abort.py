...
class CustomizedClient(BaseClient):
    ...
    def train_local_only(self, conf, device):
        ...
        for i in range(conf.local_epoch): # 注意！！conf.local_epoch按论文说的设为了1
            ...
            # print('Client {} local epoch: {}, loss: {:.4f}, acc: {:.4f}'.format(self.cid, i, self.train_loss[-1], sum(epoch_acc) / len(epoch_acc)))
    ...
    def train_DML(self, conf, device):
        ...
        for i in range(conf.local_epoch): # 注意！！conf.local_epoch按论文说的设为了1
            ...
            # print('Client {} DML epoch: {}, loss: {:.4f}, acc: {:.4f}'.format(self.cid, i, current_epoch_loss, sum(epoch_acc) / len(epoch_acc)))
            self.train_loss.append(float(current_epoch_loss))
            # 本轮epoch的loss加入到总的loss列表中
            torch.cuda.empty_cache()
        self.train_time = time.time() - start_time
        # print('Client {} DML training time: {:.4f} s'.format(self.cid, self.train_time))
